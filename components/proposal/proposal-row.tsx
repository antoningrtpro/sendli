...
import { Globe, Lock, Eye, BarChart2, Trash2, MoreHorizontal, Copy, Pencil } from "lucide-react";
...
export function ProposalRow({ id, title, published, updatedAt, status: initialStatus, amountOneShot: initialOneShot, amountMrr: initialMrr, viewCount }: ProposalRowProps) {
  ...
  function openActionsMenu() {
    ...
    setActionsPos({ top: rect.bottom + window.scrollY + 4, left: rect.right + window.scrollX - 176 });
    setActionsOpen(true);
  }
          ...
          >
            ...
            <button
              ...
              onClick={() => {
                setActionsOpen(false);
                startTransition(async () => {
                  const { id: newId } = await duplicateProposal(id);
                  toast.success("Proposition dupliquée");
                  window.location.href = `/proposals/${newId}/edit`;
                });
              }}
              disabled={isPending}
